...
public class UniGifImage : MonoBehaviour
{
    ...
    public IEnumerator SetGifFromUrlCoroutine(string url, bool autoPlay = true)
    {
        ...
        using (UnityWebRequest req = new UnityWebRequest(path))
        {
            ...
            yield return StartCoroutine(UniGif.GetTextureListCoroutine(req.downloadHandler.data, (gifTexList, loopCount, width, height) =>
            {
                if (gifTexList != null)
                {
                    ...
                    // we dont want to set width or height
                    // this.width = width;
                    // this.height = height;
