...
namespace Firesplash.GameDevAssets.SocketIOPlus.Internal
{
	...
	internal class SIODispatcher : MonoBehaviour
	{
		...
		{
			int logEleCount, dispatchCounter;
			Action dispatchAction;
            while (true)
            {
                //dispatch all pending logs before doing work
                logEleCount = logQueue.Count; //counting prevents possible deadlocks through excessive addition of elements
				...
				while (dispatchQueue.Count > 0)
				{
					...
					if (dispatchCounter++ >= maxActionsPerFrame)
					{
						break;
					}
				}
